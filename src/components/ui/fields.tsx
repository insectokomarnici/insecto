...
type FieldProps = { label: string; hint?: string; error?: string; optional?: boolean };
export function TextField({ label, hint, error, optional, id: providedId, className, ref, ...props }: InputHTMLAttributes<HTMLInputElement> & FieldProps & { ref?: Ref<HTMLInputElement> }) {
...
export function TextArea({ label, hint, error, optional, id: providedId, className, ...props }: TextareaHTMLAttributes<HTMLTextAreaElement> & FieldProps) {
  const generatedId = useId();
  const id = providedId ?? generatedId;
  const describedBy = [props["aria-describedby"], hint && `${id}-hint`, error && `${id}-error`].filter(Boolean).join(" ") || undefined;
  return <div className="field">
