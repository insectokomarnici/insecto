...
export function ContactForm({ submitContact, successMessage = "Poruka je uspešno poslata.", failureMessage = "Poruka nije poslata. Pokušajte ponovo. Uneti podaci su sačuvani u formi." }: Props) {
  ...
  return <form ref={formRef} className="contact-form" noValidate aria-label="Kontakt forma" aria-busy={busy} onSubmit={submit}>
    <div className="field-grid">
      <TextField name="name" label="Ime" optional autoComplete="name" maxLength={80} placeholder="Vaše ime" value={values.name} readOnly={busy} error={errors.name} onChange={(e) => update("name", e.target.value)} />
      <TextField name="phone" label="Telefon" type="tel" inputMode="tel" autoComplete="tel" required maxLength={40} placeholder="Broj telefona" hint="Dozvoljeni su razmaci, zagrade i početni +." value={values.phone} readOnly={busy} error={errors.phone} onChange={(e) => update("phone", e.target.value)} onBlur={() => {
        if (!busy && (phoneTouched || values.phone.trim())) { setPhoneTouched(true); setErrors((current) => ({ ...current, phone: validateContact(values).phone })); }
      }} />
    </div>
    <TextArea name="message" label="Poruka" optional rows={4} maxLength={1000} placeholder="Šta vam je potrebno?" hint="Možete navesti broj prozora i vrata. Do 1000 znakova." value={values.message} readOnly={busy} error={errors.message} onChange={(e) => update("message", e.target.value)} />
    ...
    <div><Button type="submit" loading={busy}>Pošaljite poruku</Button></div>
  </form>;
}
