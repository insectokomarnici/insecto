...
The component requests only `rating`, `userRatingCount` and `googleMapsUri`, and revalidates the response every seven days. Revalidation happens on the next request after the interval expires, so it is an interval rather than an exact scheduled time. If credentials are missing or the API request fails, it renders nothing rather than showing stale or fabricated values. The rating row includes the visible `Google recenzije` attribution and links to the source place page.
