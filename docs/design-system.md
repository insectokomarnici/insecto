...
Icon sizing is semantic: `--icon-inline` is `1rem` for icons beside text and navigation chevrons, `--icon-action` is `1.25rem` for buttons and feedback notices, `--icon-feature` is `1.5rem` for process-card heading icons, `--icon-floating-call` is `1.8rem` for the mobile floating call button, `--icon-menu` is `1rem` for the standalone mobile menu trigger, and `--icon-close` is `1rem` for the mobile menu X. The custom three-line menu path and the close path share the same visible 16px canvas and 2px visual weight. CTA buttons use Carbon's filled `PhoneFilled` icon for action emphasis, while the informational banner uses the outline `Phone` icon alongside Email and Time.
