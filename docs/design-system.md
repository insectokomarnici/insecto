...
The FAQ section uses a responsive split layout: a left information block with the shared phone CTA and a right card containing individually bordered accordion items. It collapses to a single column below the `lg` breakpoint while keeping the shared surface, typography, border, spacing and disclosure-motion tokens. The final CTA section uses compact responsive section padding, the brand surface with centered white heading text, a borderless white phone button and the shared Google rating row directly below it. Its heading remains on one line from the tablet breakpoint upward.
