...
Icon sizing is semantic: `--icon-inline` is `1rem` for icons beside text and navigation chevrons, `--icon-action` is `1.25rem` for buttons and feedback notices, `--icon-menu` is `1.25rem` for the standalone mobile menu trigger, and `--icon-close` is `1rem` for the current mobile menu X. The Banner `Close` uses `--icon-banner-close` at `1.75rem` because Carbon's standard 32px canvas contains a 16px visible path; this makes its visible X approximately `0.875rem`, matching the inline Banner icons. CTA buttons use Carbon's filled `PhoneFilled` icon for action emphasis, while the informational banner uses the outline `Phone` icon alongside Email and Time. The mobile menu close remains a separate icon treatment for later review.
