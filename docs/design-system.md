...
Icon sizing is semantic: `--icon-inline` is `1rem` for icons beside text and navigation chevrons, `--icon-action` is `1.25rem` for buttons and close actions, and `--icon-menu` is `1.25rem` for the standalone mobile menu trigger. CTA buttons use Carbon's filled `PhoneFilled` icon for action emphasis, while the informational banner uses the outline `Phone` icon alongside Email and Time. Close actions use the local full-bleed `CloseIcon` on the same 16px grid as the custom three-line menu icon, avoiding internal SVG whitespace.
