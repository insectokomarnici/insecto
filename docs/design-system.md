...
The About section uses one column below `lg` and two equal columns from `lg`, with the team image and statistic cards preceding the copy in document order. Its layout, content width, heading, body text, CTA, Google rating, borders, radii, shadows and spacing all reuse shared tokens. Statistic values use the shared lead scale for hierarchy. Below `sm`, the statistic icon containers step down to the shared `--space-8` dimension with `--icon-action` glyphs so both two-column cards remain readable without horizontal overflow at 320px. The copy groups use `--space-6`, while the two body paragraphs retain the standard stack gap.
