...
Icon sizing is semantic: `--icon-inline` is `1rem` for icons beside text, navigation chevrons and the Banner `Close`, `--icon-action` is `1.25rem` for buttons and feedback notices, `--icon-menu` is `1.375rem` for the standalone mobile menu trigger, and `--icon-close` is `1rem` for the current mobile menu X. The menu token is slightly larger because the custom three-line path has built-in side margins; its visible width now aligns with the X. CTA buttons use Carbon's filled `PhoneFilled` icon for action emphasis, while the informational banner uses the outline `Phone` icon alongside Email and Time. The mobile menu close remains a separate icon treatment for later review.
