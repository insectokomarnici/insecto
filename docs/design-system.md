...
Icon sizing is semantic: `--icon-inline` is `1rem` for icons beside text and navigation chevrons, `--icon-action` is `1.25rem` for buttons and feedback notices, `--icon-menu` is `1.25rem` for the standalone mobile menu trigger, and `--icon-close` is `1rem` for close actions. CTA buttons use Carbon's filled `PhoneFilled` icon for action emphasis, while the informational banner uses the outline `Phone` icon alongside Email and Time. Close actions use a local wrapper around Carbon's original `Close` path, cropped to its visible 16px bounds so the style stays identical without internal SVG whitespace.
