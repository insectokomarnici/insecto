...
Icon sizing is semantic: `--icon-inline` is `1rem` for icons beside text, navigation chevrons and the Banner `Close`, `--icon-action` is `1.25rem` for buttons and feedback notices, `--icon-menu` is `1rem` for the standalone mobile menu trigger, and `--icon-close` is `1rem` for the mobile menu X. The custom three-line menu path and the close path share the same visible 16px canvas and visual weight. CTA buttons use Carbon's filled `PhoneFilled` icon for action emphasis, while the informational banner uses the outline `Phone` icon alongside Email and Time.
