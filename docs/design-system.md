...
Icon sizing is semantic: `--icon-inline` is `1rem` for icons beside text, navigation chevrons and the Banner `CloseOutline`, `--icon-action` is `1.25rem` for buttons and feedback notices, `--icon-menu` is `1.25rem` for the standalone mobile menu trigger, and `--icon-close` is `1rem` for the current mobile menu X. CTA buttons use Carbon's filled `PhoneFilled` icon for action emphasis, while the informational banner uses the outline `Phone` icon alongside Email and Time. The Banner close uses Carbon's outline `CloseOutline` at the same `--icon-inline` size. The mobile menu close remains a separate icon treatment for later review.
