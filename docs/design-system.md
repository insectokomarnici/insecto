...
The FAQ section uses a responsive split layout: a left bordered card with the shared phone CTA and a right card containing individually bordered accordion items. It collapses to a single column below the `lg` breakpoint while keeping the shared surface, typography, border, spacing and disclosure-motion tokens. The final homepage section is a responsive contact-form design with a warm accent-surface panel, a left introduction and a right two-column form. Its submit handler is intentionally visual-only until real form delivery is connected.
